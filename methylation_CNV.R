...
colnames(cnv.intensity) <- paste(rownames(pdata), 'intensity', sep='.')
...
cnv.annot <- CNV.create_anno(exclude_regions=exclude.gr, detail_regions = detail_regions, chrXY=T, array_type=conumee.array[array.type])
...
#	png(file.path(qcdir, 'CNV_report', pid, paste(pid, 'whole_genome.png', sep='_')), width=2000, height=500)
	genomeplot_params <- list(cols=c('blue', 'blue', 'grey', 'orange', 'orange'))
	do.call(CNV.genomeplot, c(cnv.analysis, genomeplot_params))
#	dev.off()
...
#		png(file.path(qcdir, 'CNV_report', pid, paste(pid, '_', chr, '.png', sep='')), width=2000, height=500)
		do.call(CNV.genomeplot, c(cnv.analysis, chr=chr, genomeplot_params))
#		dev.off()
	}
	dev.off()
}
